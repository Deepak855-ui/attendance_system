...
def send_otp_teacher(email):
    ...
    For any assistance, feel free to reach out to our suppurt team.

    **Contact:** Deepak
    **Email:** imdeep2810@gmail.com
...
# app = Flask(__name__)
# print(f"secret key{secrets.token_hex(16)}")

# app.secret_key="28f8ceb2f8f6f3c3e3748250aca4e078"
# app.config['SESSION_TYPE'] = 'filesystem'  # Store session data in a file
# app.config['SESSION_PERMANENT'] = True
# app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(minutes=10)
...
# def send_otp_teacher(email):
#     """
#     Sends an OTP to the teacher's email using SMTP configuration.
#     """
...
#     if 'teacher_otp' not in session or session['teacher_otp_email'] != email:
#        print("❗ Error: OTP not generated or email mismatch.")
#        return jsonify({"error": "OTP not generated or mismatched email."}), 400
    
#     # print(f"✅ Debug: Stored teacher OTP -> {session.get('teacher_otp')}, Email -> {session.get('teacher_otp_email')}")  # Debugging

#     print(f"✅ Debug: Stored Teacher OTP -> {otp}, Email -> {email}, Expires At -> {session['teacher_otp_expiry']}")
...
#         print(f"✅ OTP teacher Generated: {otp}, Expires at: {session['teacher_otp_expiry']}")  # Debugging        
#         print(f"✅ OTP teacher Generated: {otp}, Expires at: {expiry_time}")
       
#         print(f"✅ OTP Sent Successfully to {email}")
#         return True
#     except Exception as e:
#         print("❌ Email Sending Error:", str(e)) 
#         server.quit()  # Ensure the server is closed on error
...
# def send_otp_teacher_route():
#     """
#     This route generates and sends an OTP to the teacher's email
...
#     print(f"✅ Debug: Generated Teacher OTP -> {otp}, Email -> {email}, Expires At -> {expiry_time}")
...
# # def verify_otp_teacher():
#     """
#     This route verifies the OTP entered by the teacher.
#     It checks if the OTP exists, matches, and hasn't expired.
#     """
#     data = request.get_json()
#     entered_otp = data.get('otp')

#     # ✅ Debugging: Print Entered OTP and Session Data
#     print(f"🔎 Debug: Entered OTP -> {entered_otp}")
#     print(f"🔎 Debug: Session Teacher OTP -> {session.get('teacher_otp')}")
#     print(f"🔎 Debug: Session Teacher OTP Email -> {session.get('teacher_otp_email')}")
#     print(f"🔎 Debug: Session Teacher OTP Expiry -> {session.get('teacher_otp_expiry')}")

#     # ✅ Check if OTP Exists
#     if 'teacher_otp' not in session:
#         print("❗ Error: OTP not generated or expired.")
#         return jsonify({"verified": False, "error": "OTP not generated or expired"}), 400

#     # ✅ Validate OTP Expiry
#     expiry_time_str = session.get('teacher_otp_expiry')
#     if expiry_time_str:
#         expiry_time = datetime.strptime(expiry_time_str, "%Y-%m-%d %H:%M:%S")
#         if datetime.now() > expiry_time:
...
#     if session['teacher_otp'] != entered_otp:
...
#     session.pop('teacher_otp', None)
#     session.pop('teacher_otp_email', None)
#     session.pop('teacher_otp_expiry', None)
...
#     data = request.get_json()
#     entered_otp = data.get("otp")
#     request_type = data.get("request_type")  # Expect 'registration' or 'forgot_password'

#     if not request_type or request_type not in ['registration', 'forgot_password']:
#         return jsonify({"verified": False, "error": "Invalid request type"}), 400

#     print(f"🔍 Debug: Verifying {request_type} OTP - Entered: {entered_otp}")
...
#      # ✅ Debugging: Ensure correct email and OTP are stored
#     print(f"✅ Debug: Stored OTP -> {otp}, Email -> {session['otp_email']}")  
#     # print(f"✅ Debug: Stored OTP -> {session['otp']}, Email -> {session['otp_email']}")  # Debugging
...
#        # ✅ Identify Request Type Using Referrer
#     if '/student_registration' in request.referrer:
#         request_type = 'registration'
#     elif '/forgot_password' in request.referrer:
#         request_type = 'forgot_password'
#     else:
#         return jsonify({"error": "Invalid request"}), 400

#     print(f"🔍 Debug: OTP Requested for {request_type} - Email: {email}")
...
#         # ✅ Store OTP & email in session correctly
#         session[f'{request_type}_otp'] = otp
#         session[f'{request_type}_otp_expiry'] = expiry_time.strftime("%Y-%m-%d %H:%M:%S")
#         session[f'{request_type}_otp_email'] = email  

#         print(f"✅ Debug: Stored {request_type} OTP -> {session.get('otp')}, Email -> {session.get('otp_email')}")  # Debugging
...
#     stored_otp = session.get(f'{request_type}_otp')
#     expiry_time_str = session.get(f'{request_type}_otp_expiry')
...
#     expiry_time = datetime.strptime(expiry_time_str, "%Y-%m-%d %H:%M:%S")   
#     if datetime.now() > expiry_time:
#         session.pop(f"{request_type}_otp", None)
#         session.pop(f"{request_type}_otp_expiry", None)
#         # session.pop("otp", None)  # Remove expired OTP
#         # session.pop("otp_expiry", None)
#         return jsonify({"verified": False, "error": "OTP expired!"}), 400
...
#         session[f'{request_type}_otp_verified'] = True
#         return jsonify({"verified": True})
